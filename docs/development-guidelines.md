...
:root {
  /* 累積バーの状態色 */
  --color-accum-danger: #D85A30;  /* 180分以上（オレンジ赤） */

  /* ダイアログの警告ボーダー */
  --color-dialog-danger: #D85A30;

  /* 状態スコアの点の色（app.js の scoreColor() で使用） */
  --color-state-low:    #D85A30;  /* スコア 1〜2 */
  --color-state-mid:    #888780;  /* スコア 3   */
  --color-state-high:   #185FA5;  /* スコア 4〜5 */
}
```
...
- [ ] 立ち止まるボタンでは `force` にならず `self` で開くか

**グラフ表示:**
- [ ] 期間フィルター（今日/前日/前々日/直近7日）で表示が正しく切り替わるか
- [ ] `action=rest` と `dialog_mode=first` のレコードが点として表示されるか
- [ ] 作業帯・休憩帯が背景色で正しく描画されるか
...
**例:**
```
feat: 状態スコアグラフに期間フィルターを追加する

今日・前日・前々日・直近7日の切り替えボタンを追加。
```
