...
#### 2. 作業単位ドキュメント（ステアリングファイル、`steering/[YYYYMMDD]-[開発タイトル]/`）
...
- **decisions.md** - 意思決定ログ
  - その作業内で生じた設計・UX・技術的判断を記録する
  - 選択肢・決定内容・理由・日付を必ず残す
  - docs/ への反映要否を明記する（「反映済み」「不要」「要検討」のいずれか）
  - 人手修正が発生した場合は tasklist.md の「人手修正ログ」セクションにも追記する
...
- `.steering/20260415-initial-implementation/`
- `.steering/20260420-add-book-recommendation-feature/`
- `.steering/20260425-refactor-recommendation-algorithm/`
- `.steering/20260430-fix-bug-in-recommendation-feature/`
...
アプリケーション全体の設計を定義します。
idea.mdの内容をもとに、必要なドキュメントを作成します。
...
**例:**
```bash
mkdir -p .steering/20260420-add-tag-feature
```
